export const ESinit = () => {
  const { $LCscroll } = useNuxtApp() as any
  ...
  watch(pageloaded, (next) => {
    if (next && isFirstLand) {
      ...
      if ((document.querySelector('[data-ssr]') as HTMLElement | null)?.dataset.ssr === 'false') {
...
if (import.meta.server) {
  ...
  console.log(' //\\                  //\\')
  console.log('\x1b[0m\x1b[33m%s\x1b[0m', '● SS Boilerplate is Awesome - v1.0.0 ●')
}
else {
  console.log('%c🥚 SS Boilerplate 🥚', 'border-left:10px solid #ffe800;border-color:#ffe800;background:#000000;padding:5px 15px;border-radius:5px; color:#ffffff;font-size:10px;')
}
